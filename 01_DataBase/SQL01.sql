--1. 춘 기술 대학교의 학과 이름과 계열을 표시하시오. 단, 출력 헤더는 "학과 명", "계열"으로 표시하도록한다.
...
SELECT DEPARTMENT_NO, STUDENT_NAME, DEPARTMENT_NAME, ABSENCE_YN
FROM TB_DEPARTMENT
JOIN TB_STUDENT USING(DEPARTMENT_NO)
WHERE SUBSTR(STUDENT_SSN,8,1) = '2' AND ABSENCE_YN = 'Y' AND DEPARTMENT_NAME = (SELECT DEPARTMENT_NAME
                                                                                    FROM TB_DEPARTMENT
...
--그럼 기술대학교 총장의 이름을 알아낼 수 있는 SQL문장을 작성하시오.
...
AND ABSENCE_YN  = 'N' AND EXTRACT(YEAR FROM ENTRANCE_DATE ) = 2002;
