/*
    ...
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE한 후 곧바로 탈퇴한 회원들만 따로 보관하느 테이블에
    자동으로 INSERT시켜야한다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게끔.
    입출고에 대한 데이터 기록(INSERT)될떄마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야한다.
    
    *트리거의 종류
    -SQL문의 실행시기에 따른 분류
    -BEFORE TRIGER : 내가 지정한 테이블에 이벤트가 발생되기 전에 트리거 진행
    -ARTER TRIGER : 내가 지정한 테이블에 이벤트가 발생된 후게 트리거 진행
...
--2. 상품 입출고 상세 이력 테이블 생성(TB_PROTAIL)
...
VALUES(SEQ_DECODE.NEXTVAL, 10, SYSDATE, 10, '입고');
...
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL, 20, SYSDATE, 20, '입고');
