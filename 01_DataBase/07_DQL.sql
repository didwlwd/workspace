/*
    ...
    *데이터 정의어
    오라클에서 제공하는 객체를 새로 만들고(CREATE), 구조를 변경하고(ALTER), 구조자체를 삭제(DELETE)
    즉, 실제 데이터값이 아는 규칙 자채를 정의하는 언어
...
/*
    ...
    - 테이블이란 : 행과 열로 구성되는 가장 기본적인 데이터베이스 객체
                    모든 테이터들은 테이블을 통해 저장된다.
                    (DEMS용어 중 하나로, 데이터를 일종의 표 형태로 표현한 것)
    ...
    * 자료형
    - 숫자(NUMBER)
    - 문자(CHAR(바이트크기) | VARCHAR2(바이트크기)) -> 반드시 크기를 지정해 줘야함
...
/*
    ...
    종류 : NOT NULL, UNIQUE, CHCEK, PRIMARY KEY, FOREIGN KEY
*/

/*
    *NOT NULL
    해당 컬럼에 반드시 값이 존재해야만 하는 경우(즉, 절대 NULLDL 들어오면 안되는 경우)
    삽입/수정 시 NULL값을 허용하지 않도록 제한
    
    제약조건을 부여하는 방식은 크게 2가지가 있다.(컬럼레벨방식, 테이블레벨방식)
    NOT NULL 제약조건은 무조건 컬럼레벨방식만 가능하다.
*/
...
CREATE TABLE MEM_UNIQUE(
    ...
    MEM_PWD VARCHAR2(20)NOT NULL UNIQUE,
    MEM_NAME VARCHAR2(20)NOT NULL,
    GENDER CHAR(3), --남, 여
    PHONE VARCHAR2(13),
    EMAIL VARCHAR(50),
...
INSERT INTO MEM_UNIQUE
VALUES(2, 'USER1', 'PASS2', '홍길삼','여', '010-1111-3333', 'BBB@NAVER.COM', '24/12/23');
...
INSERT INTO MEM_UNIQUE
VALUES(4, 'USER4', 'PASS4', '홍길사', NULL, NULL, NULL, NULL);
...
INSERT INTO MEM_CHECK
VALUES(2, 'USER2', 'PASS2', '홍길동','자', '010-1111-3333', 'BBB@NAVER.COM', '24/12/23');
--CHECK 제약조건 때문에 예외가 발생한다.
--GENDER 컬럼에는 CHECK제약조건을 만족하는 값을 넣어야한다,
--단, NULL은 값이 없다는 뜻이기 때문에 가능하다.

INSERT INTO MEM_CHECK
VALUES(2, 'USER3', 'PASS3', '홍길동','여', '010-1111-3333', 'BBB@NAVER.COM', '24/12/23');
----------------------------------------------------------------------------------------
...
/*
    ...
    EX) 회원번호, 학번, 군번, 부서코드, 직급코드, 주민등록번호, 택배송장번호, 예약번호...
...
INSERT INTO TB_LIKE VALUES(1, '자전거A', SYSDATE); -- 홍길순이 자건거A 좋아요 누름
...
/*
    ...
    -> 다른 테이블을 참조한다고 표현
    -> 주로 FOREING KEY 제약조건으로 인해 테이블간 관계가 형성된다.
...
CREATE TABLE MEM(
    ...
    EMAIL VARCHAR(50),
    MEM_DATE DATE, 
    GRADE_ID NUMBER REFERENCES MEM_GRADE(GARDE_CODE)
);
...
DELETE FROM MEM_GRADE
WHERE GARDE_CODE = 40;
--MEM 테이블에서 40이라는 값을 사용하고있지 않기 때문에 삭제가 된다,
...
/*
    ...
    *삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 할것인가?
    ...
    -ON DELETE SET NULL : 부모데이터 삭제시 해당 데이터를 사용하고 있는 자식테이터의 값을 NULL로 변경
...
CREATE TABLE MEM(
    ...
    EMAIL VARCHAR(50),
    MEM_DATE DATE, 
    GRADE_ID NUMBER REFERENCES MEM_GRADE(GARDE_CODE)ON DELETE SET NULL
);
...
CREATE TABLE MEM(
    ...
    EMAIL VARCHAR(50),
    MEM_DATE DATE, 
    GRADE_ID NUMBER, --REFERENCES MEM_GRADE(GARDE_CODE)ON DELETE CASCADE,
    FOREIGN KEY (GRADE_ID) REFERENCES MEM_GRADE(GARDE_CODE)
);
...
INSERT INTO MEMBER
VALUES(3,'최지원',19, DEFAULT, DEFAULT);
...
--DEPARTMENT테이블에 LOCATION_ID에 외래키 제약조건 추라
