/*
    <DCL : 데이터 제어문>
    계정에게 시스템권한 또는 객체접근 권한을 부여하건 회수하는 구문
...
--사번이 217, 216번, 214인 사람삭제
DELETE FROM EMP_01
WHERE EMP_ID IN(217,216,215);
...
--DDL문(CREATE, ALTER, DROP)을 수행하는 순간 기존 트랜잭션은 무조건 COMMIT됨(실제 DB에 변경됨)
--즉, DDL문 수행전 변경사항들이 있다면 정확하게 픽스하고 해라!
/*
    ACID속성 : 트랜잭션의 일관성과 신뢰성을 보장하기위해 지켜야하는 네가지 핵심 속성
    원자성 : 트랜잭션 내의 작업이 모두 완료(COMMIT)되거나, 모두 취소(ROLLBACK)되어야 함.
    일관송 : 트랜잭션이 실행되기 전과 후에 데이터베이스의 상태가 항상 일관성을 유지해야함.
    독립성 : 동시에 실행되는 트랜잭션이 서로에게 영향을 미치지 않도록 독립적으로 처리되어    
    자속성 : 트랜잭션이 성공적으로 완료된 후에는 시스템 장애가 발생하더라도 영구적으로 저장되어야함.
*/
