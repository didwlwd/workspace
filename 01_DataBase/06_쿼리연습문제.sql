...
SELECT EMP_NAME, TO_CHAR(SARARY,'9,000,000')
FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO,1,2) || '년' || SUBSTR(EMP_NO,3,2) || '월'
        || SUBSTR(EMP_NO,5,2) || '일' AS "생일", EXTRACT(YEAR FROM SYSDATE) -
    CASE
...
--주민번호 앞자리와 뒷자리의 합 조회 
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6)AS "앞자리",SUBSTR(EMP_NO,8,14)AS "뒷자리",
SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8)
--CONCAT(SUBSTR(EMP_NO,1,6),SUBSTR(EMP_NO,7,14))
FROM EMPLOYEE
WHERE EMP_ID = '201';
