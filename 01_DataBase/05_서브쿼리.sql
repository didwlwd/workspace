/*
    * 서브커리
    ...
    - 메인 SQL문을 위해 보조 역할을 하는 퀴리
*/

--서브퀴리 예시1) 노옹철 사원과 같은 부서에 속한 사원들 전체 조회
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
/*
    *서브쿼리의 구분
    ...
    --단일행 서브쿼리 : 서브쿼리의 조회 결과값이 오로지 한계일 때
    --다중행 서브쿼리 : 서브쿼리의 조회 결과값이 여러행일 때(여러행 한열)
    --다중열 서브쿼리 : 서브쿼리의 조회 결과값이 한행이지만 컬러가 여러개일 때
...
--4) 부서별 급여합이 가장 큰 부서의 부서코드, 급여합
    --1) 부서별 급여합이 가장 큰 부서 급여합 - A = 17700000
...
SELECT EMP_ID, EMP_NAME, PHONEM, HIRE_DATE, DEPT_TITLE
FROM EMPLOYEE
...
--1) 유재식 또는 윤은해 사원과 같은 직급인 사원들의 사변, 사원명, 직급코드, 급여조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
--2)_2) 대리직급이면서 위의 결과보다 급여가 하나라도 큰사원의 사번, 이름, 직급, 급여 조회
