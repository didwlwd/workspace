...
--2.과목 구분을 저장할 테이블을 만들려고 한다. 다음과 같은 테이블을 작성하라.
...
--6.두 테이블의 NO 컬럼과 NAME컬럼의 이름을 각각 TB_를 제외한 테이블 이름이 앞에
--붙은 형태로 변경한다.
ALTER TABLE TB_CATEGORY
...
ALTER TABLE TB_CATAGORY
    RENAME CONSTRAINT PK_CATEGORY TO PK_NAME;
    
ALTER TABLE TB_CLASS_TYPE
    RENAME CONSTRAINT SYS_C008670 TO PK_CLASS_TYPE_NO;
...
--9.TB_DEPARTMENT의 CATEGORY 컬럼이 TB_CATEGORY 테이블의 CATEGORY_NAME컬럼을 부모
--값으로 참조하도록 FOREIGN KEY를 지정하시오. 이때 KEY 이름은 FK_테이블이름_컬럼이름으로 지정한다.
...
AS ( SELECT STUDENT_NAME AS "학생이름", DEPARTMENT_NAME AS "학과이름",
            PROFESSOR_NAME AS "담당교수"
            FROM TB_STUDENT
            JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
            JOIN TB_PROFESSOR USING(PROFESSOR_NO = COACH_PROFESSOR_NO)
