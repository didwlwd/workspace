/*
    ...
    SQL문장 내에섯 변수의 정의, 조건(IF), 반복(FOR, WHILE)등을 지원하여 SQL단점을 보완
    다수의 SQL문을 한번에 실행 가능
    
    PL/SQL구조(블록구조)
    -[선언부] : DECLARE로 시작, 변수나 상수를 선언 및 초기화하는 부분
    -실행부 : BEGIN으로 시작, SQL문 또는 제어문등의 로직을 기술하는 부분
    -[예외처리부] : EXCEPTION으로 시작, 예외 발생 시 해결하기위한 구문
*/
...
--------------------------------------------------------------------------------
--1_2) 래퍼런스 타입 변수 선언및 초기화(어떤 테이블의 어떤 컬럼의 데이터타입을 참조해서 그 타입으로 지정
DECLARE
...
--입력받은 사번의 해당하는 사원의 사번, 사원명, 직급코드, 급여. 부서명 조회
...
--3)IF 조건식 1 THEN 실행내용1 ELSEIF 조건식2 THEN 실행내용...[ELSE 실행내용] END IF;
DECLARE
    SCORE NUMBER;
    GRADE VARCHAR2(1);
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('당신의 점눈는 :' || SCORE || '점이며, 학점은 ' || GRADE || '학점입니다,' );  
END;
/
...
EXCEPTION
   -- WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나구기 연산시 0으로 나눌 수 없습니다.');
