...
FROM TB_GRADE
WHERE CLASS_NO = 'C3118100' AND SUBSTR(TERM_NO,4,1) = '4' AND SUBSTR(TERM_NO,6,1) = '2'
...
--9.8번의 결과 중'인문사회' 계열에 속한 과목의 교수 이름을 찾으려고한다.
...
--"전체 평점"을 출력하는 SQL문장을 작성하시오.
--(단, 평점은 소수점 1자리까지만 반올림하여 표시한다.
SELECT G.STUDENT_NO AS "학번", STUDENT_NAME AS "학생이름",
...
--SQL문장을 작성하시오.
SELECT STUDENT_NAME AS "학생이름", TERM_NO AS "수강학기"
FROM TB_STUDENT
...
--14.춘 기술대학교 서반아어학과 학생들의 지도교수를 게시하고자 한다. 학생이름과 지도교수
--이름을 찾고 만일 지도 교수가 없는 학생일 경우 "지도교수 미지정"으로 표시하도록 하는
--SQL문을 작성하시오. 단, 출력헤더는 "학생이름", "지도교수"로 표시하며 고학번 학생이 먼저
--표시되도록한다.
...
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
JOIN TB_GRADE USING(STUDENT_NO)
WHERE SUBSTR(ABSENCE_YN,1,1) = 'N' AND POINT >= 4.0;

--16.환경조경학과 전공과목들의 과목 별 평점을 파악 할 수 있는 SQL문을 작성하시오.
SELECT DEPARTMENT_NAME, POINT
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
JOIN TB_GRADE USING(STUDENT_NO)
...
JOIN TB_GRADE USING(STUDENT_NO)
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
WHERE DEPARTMENT_NAME = '국어국문학과' AND POINT = (SELECT MAX(POINT)
                                                    FROM TB_GRADE);
...
--되도록 하고, 평점은 소수점 한 자리까지만 반올림하여 표시되도록한다.
