SELECT emp_id , emp_name , salary from employee where dept_code = 'D2';
...
/*
    ...
    From 조회할 테이블
    WHERE 조건식
    ORDER BY 정렬기준이 될 컬럼명 | 별칭 | 컬럼순번 [ASC | DESE] [NULLS FIRST | NULLS LAST]
...
*/
SELECT INSTR('AAAAAABBBCAAA', 'B' ,1) FROM DUAL;
...
*/
SELECT SUBSTR('SHOWMETHEMONEY',2) FROM DUAL;
select substr('SHOWMETHEMOBEY',7) from dual; --7번째 위치부터 끝까지
select substr('SHOWMETHEMOBEY',5,2) from dual; 
select substr('SHOWMETHEMOBEY',1,6) from dual;
select substr('SHOWMETHEMOBEY',-8,3) from dual;
...
/*
    *LPAD / RPAD
    ...
    [표현법]
    LPAD / RPAD(STRING, 최종적으로 반환알 문자열의 길이, [덧붙이고자하는 문자])
*/
...
/*
    *LTRIM/RTRIM
    ...
    문자열의 왼쪽 혹은 오른쪽에서 제거하고하는 문자들을 찾아서 제거한 나머지 문자열 반환
*/

select ltrim('      k   h   ') from dual; --앞에서부터 다른 문자가 나올 때 까지 공백제거
select ltrim('123123123kh123','123') from dual;
...
select trim(BOTH 'Z' from 'ZZZZZZZkhZZZZZZZZZ') from dual;
select trim(LEADING 'Z' from 'ZZZZZZZkhZZZZZZZ') from dual;
select trim(TRAILING 'Z' from 'ZZZZZZZZkhZZZZZZZ')from dual;
