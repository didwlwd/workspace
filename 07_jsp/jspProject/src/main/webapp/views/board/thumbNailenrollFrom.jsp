            ...
            <script>
                function loadImg(changeInput, targetImg){
                    ...
                    if(changeInput.files.length>0){//파일은 선택했을 때
                        ...
                        //해당 파일을 읽어들여 해당파일만의 고유한 url을 부여
                        //url : Base64로 인코딩된 데이터 url(파일을 실제로 표현하는 형식인 바이너리 코드를 텍스트 문자열로 인코딩한 방식)
                        reader.readAsDataURL(changeInput.files[0]);

                        //파일읽어들이기를 완료 했을 때 이벤트 핸들어를 실행
                ...
                function listviewback(){
                	location.href="${pageContext.request.contextPath}/list.th"
                }
            </script>
        </form>
    </div>
