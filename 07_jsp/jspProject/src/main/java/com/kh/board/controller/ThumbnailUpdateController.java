...
public class ThumbnailUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(JakartaServletFileUpload.isMultipartContent(request)) {
			...
			//새로운 첨부파일 o, 기존첨부파일 x	   b, refBoardNo -> board
			
			if(result > 0) {
				request.getSession().setAttribute("alertMsg","사진게시글 수정 성공");
				response.sendRedirect(request.getContextPath() + "/list.th?bno=" + b.getBoardNo());
			}else {
				if(at != null) {
					new File(savePath + at.getChangeName()).delete();
				}
				
				request.setAttribute("errorMsg", "사진게시글 수정 실패");
